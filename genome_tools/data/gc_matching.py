...
class GCTrack:
    ...
    def build(self):
        with FastaExtractor(self.fasta_file) as extractor:
            for chrom, length in tqdm(
                self.chrom_sizes.items(), total=len(self.chrom_sizes)
            ):
                ...
                gc_frac = self.bn_mean_correct_offset(
                    is_gc, self.window_size, self.flank_length
                ).astype(np.float32)
    ...
    def mask(self, exclude_regions: Union[List[GenomicInterval], pd.DataFrame]):
        ...
        for chrom, intervals in tqdm(grouped.items(), total=len(grouped)):
            ...
            exclude_windows = (
                self.bn_mean_correct_offset(mask, self.window_size) > 0
            ).astype(bool)
    ...
    def build_sampling_index(self):
        ...
        for chromosome in tqdm(self.chrom_names):
            ...
            for gc_bin, start, start_next in zip(
                unique_gc_bins, start_idx, np.append(start_idx[1:], len(gc_bins_track))
            ):
...
class GCSampler:
    ...
    def sample(
        self,
        positives: Union[List[GenomicInterval], pd.DataFrame],
        n=1,
        *args,
        **kwargs,
    ) -> pd.DataFrame:
        if self.gc_track.built is False:
            raise ValueError(
                "GC track is not built. Call `build` method for GC track first."
            )

        if not self.gc_track.masked:
            print(
                "Warning! No regions are masked in GC track. All data will be used for sampling."
            )

        positives = sanitize_bed_data(positives)
        if not self.gc_track.has_sampling_index:
            print(
                "GC track doesn't have a sampling index. Call `build_sampling_index` for GC track to enable efficent sampling"
            )
    ...
    def from_index_sampling(self, positives: list[GenomicInterval], n: int):
        gc_bins, gc_bin_counts = np.unique(
            [self.gc_track.at(positive) for positive in positives], return_counts=True
        )
        ...
        for bin, count in tqdm(zip(gc_bins, gc_bin_counts), total=len(gc_bins)):
            if len(self.gc_track.sampling_index[bin]) < count:
                print(
                    f"Warning: Not enough samples for GC bin {bin}. Requested {count}, available {len(self.gc_track.sampling_index[bin])}. Sampling with replacement"
                )
                replace = True
            else:
                replace = False
            result.append(
                self.gc_track.sampling_index[bin].sample(
                    n=count, replace=replace, random_state=self.seed
                )
            )
        ...
        for positive in tqdm(positives):
            ...
            for _ in range(n):
                interval = self._rejection_sample_one(
                    positive, gc_bin, max_iter=max_iter, same_chromosome=same_chromosome
                )
    ...
    def _rejection_sample_one(
        ...
        for _ in range(max_iter):
            ...
            sampled_center = self.rng.integers(
                self.flank_length, self.chrom_sizes[chrom] - self.flank_length
            )
            ...
            print(
                f"Warning: Maximum iterations {max_iter} reached without finding a matching region."
            )
